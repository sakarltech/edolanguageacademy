...
export default function BundleDashboard({ enrollmentId, learnerName }: BundleDashboardProps) {
  ...
  const { data: bundleData, isLoading, refetch } = trpc.student.getBundleProgress.useQuery({
    enrollmentId,
  });

  // Mutation for toggling module completion
  const toggleModule = trpc.student.toggleModuleCompletion.useMutation({
    onSuccess: () => {
      refetch();
  ...
  const levels = bundleData.levels.map((levelData) => {
    ...
    const levelTimes = {
      beginner: "5:00 PM GMT",
      intermediary: "6:00 PM GMT",
      proficient: "7:00 PM GMT",
    };
  ...
  const activeLevel = levels.find(l => l.id === bundleData.currentLevel);

  const handleToggleModule = (courseLevel: "beginner" | "intermediary" | "proficient", moduleNumber: number) => {
    toggleModule.mutate({
      enrollmentId,
      moduleNumber,
      courseLevel,
                ...
                {!isLocked && (
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {[1, 2, 3, 4].map((moduleNum) => {
                          const isCompleted = level.completedModuleNumbers.includes(moduleNum);
                          return (
                            <div
                              key={moduleNum}
                              className="flex items-center space-x-2 p-2 rounded border hover:bg-accent/50 cursor-pointer"
                              onClick={() => handleToggleModule(level.id as any, moduleNum)}
                            >
                              <Checkbox
                                checked={isCompleted}
                                onCheckedChange={() => handleToggleModule(level.id as any, moduleNum)}
                              />
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        // Navigate to the specific level's course content
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm">Complete all modules in {activeLevel?.name} to unlock the next level</p>
          </div>
          <div className="flex items-start gap-3">
            <div className="flex-shrink-0 w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-xs font-semibold text-primary">4</div>
            <p className="text-sm">Upload your assessments after completing each module</p>
          </div>
        </CardContent>
      </Card>
    </div>
